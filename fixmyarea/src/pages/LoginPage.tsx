  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user' || err.code === 'auth/cancelled-popup-request') {
        // User closed the popup window before finishing sign-in - graceful cancellation
        console.log('Google sign-in popup was closed by user.');
      } else if (err.code === 'auth/popup-blocked') {
        setError('Popup was blocked by your browser. Please allow popups for this site and try again.');
      } else if (err.code === 'auth/unauthorized-domain') {
        setError('This domain is not in the Firebase authorized list. Please use Email/Password to sign in.');
      } else if (err.code === 'auth/network-request-failed') {
        setError('Network error during sign-in. Please check your internet connection.');
      } else {
        setError(err.message || 'Google sign-in was unsuccessful. Please try again or use email sign in.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-md w-full bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Sign in to track civic issues in your neighborhood
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold transition-all shadow-xs disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {loading ? (
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="px-3 bg-white text-slate-400 font-bold uppercase tracking-wider text-[10px]">or continue with</span>
          </div>
        </div>
